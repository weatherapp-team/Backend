...
def test_get_history(monkeypatch, _):
    """
    Test getting weather history.
    Test tries to get weather two times and these results should appear in the history.
    """
...
def test_get_history_from_nonexistent_location(monkeypatch, _):
    """
    Test getting weather history from nonexistent location.
    
    This test gets weather from nonexistent location, so API should return error
    """
    user_data = {"username": "test_user", "email": "test_user@test.example", "password": "testpassword"}
    client.post("/auth/register", json=user_data)
    ...
    response1 = client.get("/weather/Bebraversity", headers={"Authorization": f"Bearer {token}"})
...
def test_get_history_not_authenticated(_):
    ...
    This test gets weather history when user is not authenticated, so API should return error message.
    """
