...
def test_get_alerts(_):
    ...
    client.request(
        ...
        headers={"Authorization": f"Bearer {token}"},
        json={
            "id": have_alerts.json()[0]["id"]
        }
    )

    one_location = client.get("/alerts", headers={"Authorization": f"Bearer {token}"})
    assert isinstance(one_location.json(), list)
    assert len(one_location.json()) == 1
    assert one_location.json()[0]["location"] == "Innopolis"

def test_get_alerts_not_authenticated(_):
    """
    Test getting alerts when not authenticated.

    This test tries to get alerts when user is not authenticated, so API should return error message.
    """
